...
class Content extends Component {
    ...
    getPosts = () => {
        axios.all([this.getAllPosts(), this.getPostCount(), this.getMediaCount()])
            .then(axios.spread((posts, number, media) => {
                this.setState({
                    posts: posts.data.Posts,
                    postNo: number.data.Posts,
                    mediaNo: media.data.media
                })
            }))
            .catch(err => {
                if (err.response) {
    ...
    getAllPosts = () => {
        return axios.get(`/posts/limit/${10}`);
    }
    getPostCount = () => {
        return axios.get('/posts/posts-count');
    }
    getMediaCount = () => {
        return axios.get('/media/media-count')
    }
                    ...
                    <div className="container">
                        <div className="row">
                            ...
                            <div className="col-sm">
                                <div className="card">
                                    <div className="card-header">
                                        <span style={{ marginRight: "5px" }}><i className="fas fa-images"></i></span>{" "}All Media
  </div>
                                    <div className="card-body">
                                        <h5 className="card-title">{this.state.mediaNo}</h5>
                                    </div>
                                </div>
                            </div>
                        </div>
